...
js = io.open(os.path.join(ROOT, '_shotchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_shotchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_shotchk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_shotchk_out.txt'), 'w', encoding='utf-8').write(txt + '\n')
print('TARGET %s\n%s' % (TARGET, txt))
