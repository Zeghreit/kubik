# -*- coding: utf-8 -*-
"""v2.58: UV-режим перестал быть рентгеном. Run: py -3 _uv58chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_uv58chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv58chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv58chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv58chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
