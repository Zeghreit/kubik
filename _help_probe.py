...
js = io.open(os.path.join(ROOT, '_help_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_help_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_help_probe.html?debug=1' % PORT
...
io.open(os.path.join(ROOT, '_help_out.txt'), 'w', encoding='utf-8').write(txt)
print(txt)
