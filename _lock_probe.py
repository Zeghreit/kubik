...
js = io.open(os.path.join(ROOT, '_lock_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_lock_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_lock_probe.html?debug=1' % PORT
...
if m:
    ...
    io.open(os.path.join(ROOT, '_lock_out.txt'), 'w', encoding='utf-8').write(txt)
else:
    io.open(os.path.join(ROOT, '_lock_out.txt'), 'w', encoding='utf-8').write(
        'NO PROBE OUTPUT\nstderr tail:\n' + (r.stderr or '')[-1500:])
