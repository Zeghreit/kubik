...
# iPhone 15 is 393 logical points wide; 375 is the narrowest phone still made.
WIDTHS = [500]   # the page loads once; the JS imposes the phone widths itself

src = io.open(os.path.join(ROOT, TARGET), encoding='utf-8').read()
js = io.open(os.path.join(ROOT, '_opbarchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_opbarchk.html'), 'w', encoding='utf-8', newline='').write(
...
for w in WIDTHS:
    RESULT.pop('txt', None)
    url = 'http://127.0.0.1:%d/_opbarchk.html?debug=1&t=%d' % (PORT, int(time.time() * 1000))
    cmd = [CHROME, '--headless=new', '--no-sandbox',
       ...
       '--enable-unsafe-swiftshader', '--use-angle=swiftshader',
           ...
           '--window-size=%d,852' % w, url]
    p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    t0 = time.time()
    while time.time() - t0 < 90 and 'txt' not in RESULT:
        time.sleep(0.3)
    p.kill()
    print('--- window %d ---' % w)
    print(RESULT.get('txt', 'NO OUTPUT'))
srv.shutdown()
