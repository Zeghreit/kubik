...
js = io.open(os.path.join(ROOT, '_field_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_field_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_field_probe.html?debug=1&t=%d' % (PORT, int(time.time()))
...
out = os.path.join(ROOT, '_field_out.txt')
if 'txt' in RESULT:
    io.open(out, 'w', encoding='utf-8').write('TARGET %s\n%s' % (TARGET, RESULT['txt']))
    print('TARGET %s\n%s' % (TARGET, RESULT['txt']))
else:
    io.open(out, 'w', encoding='utf-8').write('NO PROBE OUTPUT\n')
    print('NO PROBE OUTPUT after 150s')
