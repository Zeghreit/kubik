...
js = io.open(os.path.join(ROOT, '_histchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_histchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_histchk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
total = sfails + rfails
out = head + txt + '\nVERDICT=%s (%d failed)' % ('FAIL' if total else 'PASS', total)
io.open(os.path.join(ROOT, '_histchk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
