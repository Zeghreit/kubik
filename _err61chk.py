# -*- coding: utf-8 -*-
"""v2.58: UV-СЂРµР¶РёРј РїРµСЂРµСЃС‚Р°Р» Р±С‹С‚СЊ СЂРµРЅС‚РіРµРЅРѕРј. Run: py -3 _err61chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_err61chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_err61chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_err61chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
t0 = time.time()
while time.time() - t0 < 40 and 'txt' not in RESULT:
    time.sleep(0.3)
p.kill()
srv.shutdown()
...
io.open(os.path.join(ROOT, '_err61chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
