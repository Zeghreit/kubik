...
js = io.open(os.path.join(ROOT, '_zoom_probe.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_zoom_probe.html'), 'w', encoding='utf-8', newline='').write(
...
cmd = [CHROME, '--headless=new', '--user-data-dir=' + PROF, '--disk-cache-dir=' + os.path.join(ROOT, '_httpcache'), '--no-first-run', '--no-default-browser-check', '--disable-sync',
       ...
       '--window-size=393,852', '--virtual-time-budget=25000', '--dump-dom',
       'http://127.0.0.1:%d/_zoom_probe.html?debug=1' % PORT]
