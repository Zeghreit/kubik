...
js = io.open(os.path.join(ROOT, '_vbev_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_vbev_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_vbev_probe.html?debug=1' % PORT
cmd = [CHROME, '--headless=new', '--user-data-dir=' + PROF, '--disk-cache-dir=' + os.path.join(ROOT, '_httpcache'), '--no-first-run', '--no-default-browser-check', '--disable-sync',
       ...
       '--window-size=512,900', '--virtual-time-budget=25000', '--dump-dom', url]
...
io.open(os.path.join(ROOT, '_vbev_out.txt'), 'w', encoding='utf-8').write(txt)
print(txt)
