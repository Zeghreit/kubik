# -*- coding: utf-8 -*-
"""v2.58: UV-СЂРµР¶РёРј РїРµСЂРµСЃС‚Р°Р» Р±С‹С‚СЊ СЂРµРЅС‚РіРµРЅРѕРј. Run: py -3 _uv66chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_uv66chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv66chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv66chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv66chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
