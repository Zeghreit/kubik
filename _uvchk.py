...
js = io.open(os.path.join(ROOT, '_uvchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uvchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uvchk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uvchk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
