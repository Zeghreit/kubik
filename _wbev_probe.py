...
OUT = sys.argv[2] if len(sys.argv) > 2 else '_wbev_out.txt'
...
PROF = tempfile.mkdtemp(prefix='_wbev_prof_')
CHROME = r'C:\Program Files\Google\Chrome\Application\chrome.exe'
PORT = 8871

src = io.open(os.path.join(ROOT, TARGET), encoding='utf-8').read()
js = io.open(os.path.join(ROOT, '_wbev_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_wbev_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_wbev_probe.html?debug=1' % PORT
