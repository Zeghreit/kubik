...
# A fresh profile of this probe's own; a shared http cache beside it, because
# the app pulls three.js from a CDN on every load.
...
js = io.open(os.path.join(ROOT, '_sharechk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_sharechk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_sharechk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
txt = RESULT.get('txt', 'NO OUTPUT after 150s')
io.open(os.path.join(ROOT, '_sharechk_out.txt'), 'w', encoding='utf-8').write(txt + '\n')
print('TARGET %s\n%s' % (TARGET, txt))
