# -*- coding: utf-8 -*-
"""v2.70: Relax - the inside of an island, its outline held still.
Run: py -3 _uv270chk.py [target.html] [W] [H]   (default 412x915, his phone)"""
...
js = io.open(os.path.join(ROOT, '_uv270chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv270chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv270chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv270chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
