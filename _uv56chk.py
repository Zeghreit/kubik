# -*- coding: utf-8 -*-
"""v2.56: Merge islands с настоящим совмещением UV. Run: py -3 _uv56chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_uv56chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv56chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv56chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv56chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
