...
PORT = 8871
JS = sys.argv[1] if len(sys.argv) > 1 else '_out_probe.js'
OUT = sys.argv[2] if len(sys.argv) > 2 else '_out_out.txt'

src = io.open(os.path.join(ROOT, 'index.html'), encoding='utf-8').read()
js = io.open(os.path.join(ROOT, JS), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_out_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_out_probe.html?debug=1' % PORT
