...
Run: py -3 _uv69chk.py [target.html] [W] [H]   (default 412x915, his phone)"""
...
js = io.open(os.path.join(ROOT, '_uv69chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv69chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv69chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv69chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
