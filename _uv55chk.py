# -*- coding: utf-8 -*-
"""v2.55: выделение островов и Face-режим. Run: py -3 _uv55chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_uv55chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv55chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv55chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv55chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
