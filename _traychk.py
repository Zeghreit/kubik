...
js = io.open(os.path.join(ROOT, '_traychk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_traychk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_traychk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
print('TARGET %s\n%s' % (TARGET, RESULT.get('txt', 'NO OUTPUT after 90s')))
