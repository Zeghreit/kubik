...
js = io.open(os.path.join(ROOT, '_neochk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_neochk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_neochk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_neochk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
