...
js = io.open(os.path.join(ROOT, '_piv_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_piv_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_piv_probe.html?debug=1' % PORT
...
if m:
    ...
    io.open(os.path.join(ROOT, '_piv_out.txt'), 'w', encoding='utf-8').write(txt)
    print('wrote _piv_out.txt', len(txt))
else:
