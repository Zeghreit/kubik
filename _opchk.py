...
# v2.38: does an immediate edit have one ending.
#
...
OUT = sys.argv[2] if len(sys.argv) > 2 else '_opchk_out.txt'
...
PROF = tempfile.mkdtemp(prefix='_opchk_prof_')
CHROME = r'C:\Program Files\Google\Chrome\Application\chrome.exe'
PORT = 8881

src = io.open(os.path.join(ROOT, TARGET), encoding='utf-8').read()
js = io.open(os.path.join(ROOT, '_opchk.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_opchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_opchk.html?debug=1' % PORT
