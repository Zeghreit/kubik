...
js = io.open(os.path.join(ROOT, '_facemap.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_facemap.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_facemap.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_facemap_out.txt'), 'w', encoding='utf-8').write(txt + '\n')
print('TARGET %s\n%s' % (TARGET, txt))
