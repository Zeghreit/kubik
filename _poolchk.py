...
js = io.open(os.path.join(ROOT, '_poolchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_poolchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_poolchk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_poolchk_out.txt'), 'w', encoding='utf-8').write(txt + '\n')
print('TARGET %s\n%s' % (TARGET, txt))
