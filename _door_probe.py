...
js = io.open(os.path.join(ROOT, '_door_probe.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_door_probe.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_door_probe.html?debug=1' % PORT
...
io.open(os.path.join(ROOT, '_door_out.txt'), 'w', encoding='utf-8').write(txt)
print(txt)
