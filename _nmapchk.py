...
OUT = sys.argv[2] if len(sys.argv) > 2 else '_nmapchk_out.txt'
...
PROF = tempfile.mkdtemp(prefix='_nmapchk_prof_')
CHROME = r'C:\Program Files\Google\Chrome\Application\chrome.exe'
PORT = 8875

src = io.open(os.path.join(ROOT, TARGET), encoding='utf-8').read()
js = io.open(os.path.join(ROOT, '_nmapchk.js'), encoding='utf-8').read()
assert '</body>' in src
io.open(os.path.join(ROOT, '_nmapchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_nmapchk.html?debug=1' % PORT
...
# THE PICTURES, NOT ONLY THE NUMBERS. Twice on this project a measurement came
# back healthy on a render that was visibly wrong (a2.25's four corner dots,
# v2.9's halo), and both times only the screenshot said so. read_file returns
# a .png as an image.
import base64
ms = re.search(r'<pre id="probeShots">(.*?)</pre>', r.stdout, re.S)
if ms:
    for line in _html.unescape(ms.group(1)).splitlines():
        if '\t' not in line:
            continue
        name, url = line.split('\t', 1)
        if not url.startswith('data:image/png;base64,'):
            continue
        out = os.path.join(ROOT, '_nmapshot_' + name.replace('+', '_and_') + '.png')
        io.open(out, 'wb').write(base64.b64decode(url.split(',', 1)[1]))
        print('shot ->', os.path.basename(out))
print(txt)
