# -*- coding: utf-8 -*-
"""v2.53: 2D-развёртка как вид. Run: py -3 _uv53chk.py [target.html]"""
...
js = io.open(os.path.join(ROOT, '_uv53chk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv53chk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv53chk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv53chk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
print('TARGET %s\n%s' % (TARGET, out))
