# -*- coding: utf-8 -*-
"""v2.69: the 2D UV view uses the whole screen.
Run: py -3 _uv69cchk.py [target.html] [W] [H]   (default 412x915, his phone)"""
...
js = io.open(os.path.join(ROOT, '_uv69cchk.js'), encoding='utf-8').read()
io.open(os.path.join(ROOT, '_uv69cchk.html'), 'w', encoding='utf-8', newline='').write(
...
url = 'http://127.0.0.1:%d/_uv69cchk.html?debug=1&t=%d' % (PORT, int(time.time()))
...
io.open(os.path.join(ROOT, '_uv69cchk_out.txt'), 'w', encoding='utf-8').write(out + '\n')
